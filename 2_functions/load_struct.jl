...
# ==============================================================================
    struct Instance
        ...
        function Instance(pu_fname,cf_fname,bound_fname,cf_unc_fname,beta,targets,gridgraph)
            ...
            # Check if the input data files exist
            for f in vcat(pu_fname,cf_fname,bound_fname,cf_unc_fname)
                if !isfile(f)
                    println("WARNING! File $(f) is missing")
                end
            end
            ...
            N_sc = size(CSV.read(cf_unc_fname[1], DataFrame, header=1, delim=","))[2]-2

            # Problem ranges
            PlanningUnits = collect(1:N_pu)
            ConservationFeatures = collect(1:N_cf)
            Scenarios = collect(1:N_sc)
            ...
            # Targets and amount of conservation features
            ...
            AmountScenarios = zeros(N_cf,N_pu,N_sc)
            for i in 1:N_cf
                AmountScenarios[i,:,:] .= CSV.read(cf_unc_fname[i], DataFrame, header=1, delim=",")[1:N_pu,3:(N_sc+2)]
            end
            
            # Probability of scenarios
            Probabilities = zeros(N_sc,1)
            for s in 1:N_sc
                Probabilities[s] = 1/N_sc
            end
            ...
            # constructor
            new(N_pu,N_cf,N_bd,N_sc,PlanningUnits,ConservationFeatures,Scenarios,Cost,IsLockedOut,LockedOut,Amount,Targets,AmountScenarios,Probabilities,BoundaryLength,BoundaryCorrection,beta)
