...
# ==============================================================================
    ...
    function visualisation_coverage_histograms(reserve,instance,plot_opt,i,pic_name)
        ...
        scenarios_reserve_coverage = zeros(N_sc,1)
        
        for s in 1:N_sc
            scenarios_reserve_coverage[s] = sum(AmountScenarios[i,1:N_pu,s] .* Reserve)  
        end
        fails = sum(scenarios_reserve_coverage .< Targets[i])

        Plots.plot(title="Histogram of CF$(i) coverage for all scenarios ($(fails) fails)", xlabel="Reserve coverage",ylabel="Number of scenarios",
                    size=(png_res_width,png_res_height),legend=false)
        Plots.histogram!(scenarios_reserve_coverage)
        Plots.vline!([Targets[i]],linecolor=:red,lw=2)
        png("$(pic_dir)/$(pic_name)")
        
        return scenarios_reserve_coverage, fails
    end
